...
from .models import (
    QCProject, QCSite, ChecklistTemplate, ChecklistInstance, 
    ChecklistItemResult, QCIssue, QCAuditLog
)
...
from .permissions import IsQCLoggedIn, CanFillChecklist, CanVerifyChecklist, CanAudit

# --- ViewSets ---
class QCProjectViewSet(viewsets.ModelViewSet):
    queryset = QCProject.objects.all()
    serializer_class = QCProjectSerializer
    permission_classes = [IsQCLoggedIn]

class QCSiteViewSet(viewsets.ModelViewSet):
...
class QCIssueViewSet(viewsets.ModelViewSet):
    queryset = QCIssue.objects.all()
    serializer_class = QCIssueSerializer
    permission_classes = [IsQCLoggedIn]

# --- Custom Action Views ---
class ChecklistItemVerifyView(APIView):
    permission_classes = [IsQCLoggedIn, CanVerifyChecklist]

    def post(self, request, item_id):
        try:
            item = ChecklistItemResult.objects.get(id=item_id)
            status_val = request.data.get("status")  # Passed / Failed
            ...
            QCAuditLog.objects.create(
                user=request.user,
                action="VERIFY_ITEM",
                object_type="ChecklistItemResult",
                object_id=item.id,
                new_value=status_val
            )
            return Response({"message": "Item verified successfully"})
        except ChecklistItemResult.DoesNotExist:
            return Response({"error": "Item not found"}, status=status.HTTP_404_NOT_FOUND)

class ChecklistItemReconfirmView(APIView):
    permission_classes = [IsQCLoggedIn, CanFillChecklist]

    def post(self, request, item_id):
        # Reconfirm logic for L1 Inspector
        return Response({"message": "Item resubmitted for reconfirmation"})

class AuditRandomCheckView(APIView):
    permission_classes = [IsQCLoggedIn, CanAudit]
